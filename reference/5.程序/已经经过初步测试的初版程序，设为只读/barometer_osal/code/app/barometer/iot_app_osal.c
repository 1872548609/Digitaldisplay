...
const pTaskEventHandlerFn tasksArr[] = 
{
	...
	iot_app_barometer_disp_process_event,
	#else 
	#error "The IOT_APP_BAROMETER_DISP_H is removed."
	#endif 
    
    #ifdef _IOT_APP_DATA_PROCESSING_AND_OUTPUT_CONTROL_H
	iot_app_data_processing_and_output_control_process_event,
	#else 
	#error "The IOT_APP_DATA_PROCESSING_AND_OUTPUT_CONTROL_H is removed."
	#endif 
    
    #ifdef _IOT_APP_COMMON_DATA_H
    iot_app_common_data_process_event,
    #else 
    #error "The IOT_APP_COMMON_DATA_H is removed."
    #endif 
};
...
{
    #ifdef _IOT_APP_DATA_PROCESSING_AND_OUTPUT_CONTROL_H
	iot_app_data_processing_and_output_control_poll();
	#endif 
}
...
void osalInitTasks( void )
{
	...
	#ifdef _IOT_APP_BAROMETER_DISP_H
	iot_app_barometer_disp_init( taskID++ );
	#endif 
    
    #ifdef _IOT_APP_DATA_PROCESSING_AND_OUTPUT_CONTROL_H
    //= 这个事件有个2ms的任务，需要把优先级放在最下面，不然会频繁占用资源
	iot_app_data_processing_and_output_control_init( taskID++ );
	#endif 
	
    #ifdef _IOT_APP_COMMON_DATA_H
    iot_app_common_data_init( taskID++ );
    #endif 
}
