...
void HalFlashWrite(uint16 addr, uint8 *buf, uint16 cnt)
{
    ...
    for (uint16_t i = 0; i < words; i++)
    {
        ...
        std_flash_clear_flag(FLASH_FLAG_ALL_ERR);   //-- 清除错误标志
            
        std_flash_unlock();                         //-- Flash控制寄存器解锁
        if (std_flash_get_lock_status() == false)
        {
            status = std_flash_word_program(flash_addr, word_write);
        }
        std_flash_lock();                           //-- Flash控制寄存器加锁
...
void HalFlashErase(uint8 pg)
{
    ...
    if (std_flash_get_lock_status() == false)
    {
        std_flash_erase(FLASH_ERASE_PAGE, pg);
    }
    std_flash_lock();                           //-- Flash控制寄存器加锁
}
