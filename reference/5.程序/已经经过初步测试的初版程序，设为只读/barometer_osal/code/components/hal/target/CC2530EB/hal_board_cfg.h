...
{                                                                \
  uint16 i;                                                      \
                                                                 \
  /* 启动时钟源 */                                               \
  SLEEPCMD &= ~OSC_PD;                       /* 清除OSC_PD位(0x04)，开启16MHz RC振荡器和32MHz外部晶振电源 */ \
                                             /* 操作SLEEP寄存器，给16MHz高速振荡器和32M外部晶体振荡上电 */ \
                                             /* #define OSC_PD 0x04 */ \
                                                                 \
  /* 等待32MHz晶振稳定 */                                        \
  while (!(SLEEPSTA & XOSC_STB));            /* 轮询SLEEPSTA寄存器的XOSC_STB位，直到32MHz外部晶振稳定 */ \
                                             /* SLEEPSTA寄存器中的XOSC_STB位表示外部晶振是否稳定 */ \
  asm("NOP");                                /* 插入空操作指令，芯片bug workaround（硬件缺陷修复） */ \
  for (i=0; i<504; i++) asm("NOP");          /* 精确延时63us（根据数据手册要求的最小64us延时） */ \
                                             /* 注释说明：32MHz下每个NOP约125ns，504次≈63us（手册要求64us）*/ \
                                                                 \
  /* 切换系统主时钟 */                                           \
  CLKCONCMD = (CLKCONCMD_32MHZ | OSC_32KHZ); /* 设置CLKCONCMD寄存器： */ \
                                             /* 1. 选择32MHz外部晶振作为系统时钟源 */ \
                                             /* 2. 选择32.768kHz外部晶振作为32K时钟源 */ \
                                                                 \
  /* 等待时钟切换完成 */                                         \
  while (CLKCONSTA != (CLKCONCMD_32MHZ | OSC_32KHZ)); /* 轮询CLKCONSTA寄存器，确认时钟设置已生效 */ \
                                                      /* 必须等待硬件完成时钟源切换 */ \
                                                                 \
  /* 关闭未使用的时钟源 */                                       \
  SLEEPCMD |= OSC_PD;                        /* 重新设置OSC_PD位，关闭16MHz RC振荡器以省电 */ \
                                             /* 注释说明：此时仅使用32MHz外部晶振 */ \
                                                                 \
  /* 时钟配置流程总结： */                                       \
  /* 1. 所有时钟源上电 */                                        \
  ...
  /* 6. 关闭未使用的时钟源 */                                    \
                                                                 \
  /* 开启缓存预取功能（提升CPU性能）*/                           \
  PREFETCH_ENABLE();                         /* 调用宏开启代码预取模式（可能操作MEMCTR寄存器）*/ \
                                                                 \
  /* 配置GPIO方向 */                                             \
  LED1_DDR |= LED1_BV;                       /* 设置LED1引脚为输出模式（操作数据方向寄存器）*/ \
  LED2_DDR |= LED2_BV;                       /* 设置LED2引脚为输出模式 */ \
  LED3_DDR |= LED3_BV;                       /* 设置LED3引脚为输出模式 */ \
                                                                 \
  /* 配置三态输入（高阻态）*/                                     \
  P0INP |= PUSH2_BV;                         /* 设置PUSH2按钮引脚为三态输入（禁用内部上拉/下拉）*/ \
}
