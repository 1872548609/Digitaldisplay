...
void HalLedInit(void)
{
    ...
    uint8_t no_default_bits = 0;

    #ifdef HAL_LED1_INIT
    HAL_LED1_INIT();
    #ifdef HAL_LED1_DEFAULT_STATUS
    HAL_LED1_DEFAULT_STATUS();
    #else
    no_default_bits |= HAL_LED1_BIT;
    #endif
    #endif

    #ifdef HAL_LED2_INIT
    HAL_LED2_INIT();
    #ifdef HAL_LED2_DEFAULT_STATUS
    HAL_LED2_DEFAULT_STATUS();
    #else
    no_default_bits |= HAL_LED2_BIT;
    #endif
    #endif

    #ifdef HAL_LED3_INIT
    HAL_LED3_INIT();
    #ifdef HAL_LED3_DEFAULT_STATUS
    HAL_LED3_DEFAULT_STATUS();
    #else
    no_default_bits |= HAL_LED3_BIT;
    #endif
    #endif

    #ifdef HAL_LED4_INIT
    HAL_LED4_INIT();
    #ifdef HAL_LED4_DEFAULT_STATUS
    HAL_LED4_DEFAULT_STATUS();
    #else
    no_default_bits |= HAL_LED4_BIT;
    #endif
    #endif

    #ifdef HAL_LED5_INIT
    HAL_LED5_INIT();
    #ifdef HAL_LED5_DEFAULT_STATUS
    HAL_LED5_DEFAULT_STATUS();
    #else
    no_default_bits |= HAL_LED5_BIT;
    #endif
    #endif

    #ifdef HAL_LED6_INIT
    HAL_LED6_INIT();
    #ifdef HAL_LED6_DEFAULT_STATUS
    HAL_LED6_DEFAULT_STATUS();
    #else
    no_default_bits |= HAL_LED6_BIT;
    #endif
    #endif

    #ifdef HAL_LED7_INIT
    HAL_LED7_INIT();
    #ifdef HAL_LED7_DEFAULT_STATUS
    HAL_LED7_DEFAULT_STATUS();
    #else
    no_default_bits |= HAL_LED7_BIT;
    #endif
    #endif

    #ifdef HAL_LED8_INIT
    HAL_LED8_INIT();
    #ifdef HAL_LED8_DEFAULT_STATUS
    HAL_LED8_DEFAULT_STATUS();
    #else
    no_default_bits |= HAL_LED8_BIT;
    #endif
    #endif

    /* 初始化所有LED为关闭状态 */
    HalLedSet(no_default_bits, HAL_LED_MODE_OFF);
    #endif /* HAL_LED */
        ...
        if (percent < 100) // 只有占空比小于100%时才需要闪烁，占空比 <100%：需要闪烁逻辑。占空比 ≥100%：直接点亮LED（HalLedSet(leds, HAL_LED_MODE_ON)）。
        {
            ...
            while (leds)//如果leds存在
            {
                if (leds & led)//leds & led 检查当前LED是否需要配置。
                {
                    ...
                    if (!numBlinks) 
                    {
                        sts->mode |= HAL_LED_MODE_FLASH;              /* 0表示连续闪烁 */
                    }
...
void HalLedOnOff (uint8 leds, uint8 mode)
{
    ...
    if (leds & HAL_LED_1)
    {
        ...
        {
            #ifdef HAL_LED1_TURN_ON
            HAL_LED1_TURN_ON(); // 打开LED1
            #endif
        }
        else
        {
            #ifdef HAL_LED1_TURN_OFF
            HAL_LED1_TURN_OFF(); // 关闭LED1
            #endif
        }
    }
    #endif
    ...
    if (leds & HAL_LED_2)
    {
        if (mode == HAL_LED_MODE_ON)
        {
            #ifdef HAL_LED2_TURN_ON
            HAL_LED2_TURN_ON(); // 打开LED2
            #endif
        }
        else
        {
            #ifdef HAL_LED2_TURN_OFF
            HAL_LED2_TURN_OFF(); // 关闭LED2
            #endif
        }
    }
    #endif
    ...
    if (leds & HAL_LED_3)
    {
        if (mode == HAL_LED_MODE_ON)
        {
            #ifdef HAL_LED3_TURN_ON
            HAL_LED3_TURN_ON(); // 打开LED3
            #endif
        }
        else
        {
            #ifdef HAL_LED3_TURN_OFF
            HAL_LED3_TURN_OFF(); // 关闭LED3
            #endif
        }
    }
    #endif
    ...
    if (leds & HAL_LED_4)
    {
        if (mode == HAL_LED_MODE_ON)
        {
            #ifdef HAL_LED4_TURN_ON
            HAL_LED4_TURN_ON(); // 打开LED4
            #endif
        }
        else
        {
            #ifdef HAL_LED4_TURN_OFF
            HAL_LED4_TURN_OFF(); // 关闭LED4
            #endif
        }
    }
    #endif
...
void HalLedEnterSleep( void )
{
    ...
    #ifdef HAL_LED1_STATE
    HalSleepLedState |= HAL_LED1_STATE(); // 保存LED1状态
    #endif

    #ifdef HAL_LED2_STATE
    HalSleepLedState |= HAL_LED2_STATE() << 1; // 保存LED2状态（左移1位）
    #endif

    #ifdef HAL_LED3_STATE
    HalSleepLedState |= HAL_LED3_STATE() << 2; // 保存LED3状态（左移2位）
    #endif

    #ifdef HAL_LED4_STATE
    HalSleepLedState |= HAL_LED4_STATE() << 3; // 保存LED4状态（左移3位）
    #endif
