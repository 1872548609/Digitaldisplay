...
uint16_t ECO_delay_tick =0;//记录时间戳
uint8_t ECO_delay_start=0;
uint8_t ECO_delay_flag=0;
//通用延时======================================================
...
Flash_CMD Flash_CmdTran = Flash_ReadInit; //默认读取预设
//串口校准标志=========================
	 ...
	 switch(cmd)
		{
			...
			case Flash_Write:{
			 Flash_Write_SetValue();  //写入前提是改变
			 Flash_CmdTran = Flash_NoCmd;
			}break;	
			case Flash_ReadInit:{
...
{
	  static uint8_t threered_pwmval = 0;
			static uint8_t onered_pwmval = 0;
			static uint8_t doubleged_pwmval = 0;
			static uint8_t oneged_pwmval = 0;
	 ...
	 if(!cmd && runmode == Rstate_Run_mode && manu1_devicestate == status_run)
		...
		if(ECO_delay_flag)
		{
			uint8_t ecolevel=0;
				...
				if(threered_pwmval){	threered_pwmval = ecolevel;	}
				if(onered_pwmval)  { onered_pwmval = ecolevel;			}
				if(doubleged_pwmval) { doubleged_pwmval = ecolevel;}
				if(oneged_pwmval)  { oneged_pwmval = ecolevel;			}
		}	

		DIV_Pwn_Set(&PWM_THREERED,100,threered_pwmval,PWM_ID_TR);
		DIV_Pwn_Set(&PWM_ONERED,100,		onered_pwmval,PWM_ID_OR);
		DIV_Pwn_Set(&PWM_DOUBGED,100,	doubleged_pwmval,PWM_ID_DG);
		DIV_Pwn_Set(&PWM_ONEGED,100,		oneged_pwmval,PWM_ID_OG);
	
}
void ColorDiv(void)        //颜色亮度控制
{	
...
void MiniScreenOutset(void) //小屏输出显示
{
  //out12状态显示控制
	if(out1_state)
	{
...
void Poweron_initialization(void)
{
	    ...
	    ColorCtr_CmdTran=ColorCtr_init;    //初始化颜色
					...
					{ 	
							delay_start=0;
						 Main_Disp_CmdTran=MainDisp_Clear;	   //显示初始化界面
							manu1_devicestate= status_run;
		   ...
		   if(timer_10ms_execute)
					{
						...
						Flash_Ctr(Flash_CmdTran);
						Data_Processing(DataProcess_CmdTran);//数据处理
						mainTask();  					//主任务执行 
						OutPut_Ctr(&OutPut_CmdTran); //输出控制,为了在外面也能输出
						ColorCtr(ColorCtr_CmdTran);//颜色设置命令
						MiniScreenOutset();//小屏输出显示
