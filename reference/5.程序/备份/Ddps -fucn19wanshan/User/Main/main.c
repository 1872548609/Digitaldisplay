		...
		switch(compsestatus1)   //在run模式下按下mode显示不同的设置选项
		{
					case compare1_EASY:{
						    if(unitchange_pressure_value>(P1_Value+diflevel_value))//大于应差才输出
										...
										else if(unitchange_pressure_value<(P1_Value))
										{
														out1_state =0;
									...
									switch(compsestatus2)
									{
										case compare2_EASY:{																												
															//双EASY应差模式
											   if(unitchange_pressure_value>(P2_Value+diflevel_value))
															{
																			out2_state=1;
															...
															else if(unitchange_pressure_value<(P2_Value))
															{
			                 out2_state=0;
			                 ...
			                 out2_state=0;
										...
										case compare2_WCMP:{
														  if(unitchange_pressure_value>(Lo2_Value+diflevel_value)&&unitchange_pressure_value<(Hi2_Value+diflevel_value))
															{
																			out2_state=1;
					...
					case compare1_HSY:{
						      if(unitchange_pressure_value>(Hi1_Value))//大于应差才输出
										{
													out1_state=1;        
					...
					case compare1_WCMP:{
						   if(unitchange_pressure_value>(Lo1_Value+diflevel_value)&&unitchange_pressure_value<(Hi1_Value+diflevel_value))
									{
													out1_state=1;
...
{  
   	...
   	if(key_management.WichKeyDown)//按键没松开,连按设置
			...
			else//按键释放
			{
				...
				if(ifdisupordown)
				{
					 DIV_Disp_AllPoint(SecondScreen);
						ifdisupordown=0;
				}
    ...
    if (ADC_ReadIntFlag(ADC_INT_FLAG_CS) == SET)
    {
        ADC_ClearIntFlag(ADC_INT_FLAG_CS);
					   ...
					   adc_temp1=(uint16_t)(adc_sum / WINDOW_SIZE);
								...
								if(adc_temp1>adcData&&adc_temp1-adcData>3)
								{
											adc_sum1 += adc_temp1;  
											adc_count++;
								}
					   else if(adc_temp1<adcData&&adcData-adc_temp1>3)
								{
										 	adc_sum1 += adc_temp1;  
											adc_count++;
								}
