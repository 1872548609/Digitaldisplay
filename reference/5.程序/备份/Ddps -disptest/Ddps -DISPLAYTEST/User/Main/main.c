...
int main(void)
{
    APM_EVAL_Init();
				...
				//设置LCD偏置 4com
						HAL_Disp_W_Cmd(cmdid,BIAS_1_2_4_COMS);
						//打开LCD偏置
    ...
    {					
					   ...
					   uint8_t OneFrame[2] = {0xd,0xd};
					
			//	HAL_Disp_WrSeriesData(dataid,0x02,OneFrame,2);
								
				HAL_Disp_W_Data(dataid,0x02,0xd);
					HAL_Disp_W_Data(dataid,0x03,0xd) ;
