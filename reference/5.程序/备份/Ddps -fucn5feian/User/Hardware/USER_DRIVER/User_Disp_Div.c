...
void DIV_Disp_SecondDispSetwithHunit(uint8_t sign,uint16_t value,uint8_t unit)
{
	  switch(unit)
			{
					case 0x01:{   
					     if(sign&&value<1000){
														...
														DIV_Disp_floatNum(SecondScreen,(float)value/10);
										}else if(sign==0&&value<1000){
														DIV_Disp_floatNum(SecondScreen,(float)value/10);
              ...
              DIV_Disp_Symbol(NIXIE8,Symbol_Negativeone);
											   if(value/100%10)
														{
														   DIV_Disp_floatNum(SecondScreen,(float)value/10-100);
														}
														else
														{
...
void DIV_Disp_SecondDispSetnum(uint8_t sign,uint16_t value)
{
			if(sign)
			{
				   if(value<10)
								{
										DIV_Disp_Snprintf(SecondScreen," 00%d",value);
								}
								else if(value>=10&&value<100)
								{
											DIV_Disp_Snprintf(SecondScreen," 0%d",value);
								}
								else if(value>=100&&value<1000)
								{
											DIV_Disp_Snprintf(SecondScreen," %d",value);
								}																				
...
void DIV_Disp_floatNum(uint8_t screen,float num)   //显示浮点数
{
	  ...
	  int length = snprintf(buffer, sizeof(buffer), "%.1f", num);
			...
			if(length<10)
			{
        DIV_Disp_FloatByString(screen,buffer);
			}
