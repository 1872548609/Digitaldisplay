...
//可查询延时===========================================
typedef struct{
   uint32_t Encheck_Delay_Tick;
			uint32_t Encheck_Delay_Starttime;
			uint32_t Encheck_Delay_Nowtime;
}Encheck_Delay;
Encheck_Delay encheck_delay;
#define Delayreset(); encheck_delay.Encheck_Delay_Tick=0;

#define Delayset();  encheck_delay.Encheck_Delay_Tick=0;    \
																				encheck_delay.Encheck_Delay_Starttime=encheck_delay.Encheck_Delay_Tick;

#define Delaycheck(x) encheck_delay.Encheck_Delay_Nowtime=encheck_delay.Encheck_Delay_Tick;  \
																						if(	encheck_delay.Encheck_Delay_Nowtime-encheck_delay.Encheck_Delay_Starttime>x)

//10ms执行标志=========================================
uint8_t DIV_10MS;

//设定值===============================================
volatile uint16_t P1_Value = 0;
volatile uint16_t P2_Value = 0;
volatile uint16_t Hi1_Value = 0;
volatile uint16_t Lo1_Value = 0;		
volatile uint16_t Hi2_Value = 0;
volatile uint16_t Lo2_Value = 0;
																						
volatile uint16_t testvalue=0;
...
volatile uint8_t Set_2sLongPressEv =0;		//2是key2 3是key3	哪个按键连按了加速	
...
//长按设置
void Perform_numerical_Edition(uint16_t * temp);

//显示输出预设
void Show_Presets (void);

//短按设置
void Perform_numerical_Edition_ShortPress(uint8_t setmode,uint16_t * value);

//设置对应选择
void Set_corresponding_option(uint8_t addsub,uint8_t runsetswitch);

//run模式下显示mode
void Runstate_Disp_runset(void);
...
{	
	switch(manu1_devicestate)
	{
		...
		case status_run:{
			    ...
			    Runstate_Disp_runset();    //显示设置哪个模式
			
							
								switch(compsestatus1)      //长按设置当前项
								{
									case compare1_EASY:{
									    switch(compsestatus2){
														case compare2_EASY:{
														      Perform_numerical_Edition(&testvalue);
														
														}break;
														case compare2_WCMP:{}break;
														case compare2_HSY:{}break;
														case compare2_off:{}break;
													}
									}break;
									case compare1_HSY:{}break;
									case compare1_WCMP:{}break;
								}
...
void Set_corresponding_option(uint8_t addsub,uint8_t runsetswitch)
{
   ...
   switch(runsetswitch)
			{
				case set_P1:{
									Perform_numerical_Edition_ShortPress(i,&P1_Value);
				}break;
				case set_P2:{
									Perform_numerical_Edition_ShortPress(i,&P2_Value);
				}break;
				case set_Hi2:{
									Perform_numerical_Edition_ShortPress(i,&Hi2_Value);
				}break;
				case set_Lo2:{
									Perform_numerical_Edition_ShortPress(i,&Lo2_Value);
				}break;
				case set_Hi1:{
									Perform_numerical_Edition_ShortPress(i,&Hi1_Value);
				}break;
				case set_Lo1:{
									Perform_numerical_Edition_ShortPress(i,&Lo1_Value);
				}break;
			}
}
...
void Perform_numerical_Edition_ShortPress(uint8_t setmode,uint16_t * value)
{
	if(setmode)  //加
	{
	 if(Set_sign){
		...
		DIV_Disp_SecondDispSetnum(Set_sign,* value);
	}
	else //减法
	{
		 if(Set_sign){
...
#define addmax 1050 //设置上限
#define downmax 1050 

uint8_t addordownflag = add; //增加或减少

uint16_t addtime = 200;   //增加时间间隔 200ms

volatile uint16_t presstime=0;   //时间戳

volatile uint16_t addcount =0; //计时时间间隔

volatile uint8_t addflag=0;	//自增标志

volatile uint8_t addamount = adddeta1;//自增量

uint16_t nowvalue =0; //查看当前自增到多少
				  ...
				  if(addflag)
					{
						 ...
						 if(Set_sign) { //如果是正数
										if(addamount==adddeta1)//如果用低速
										{
														if(value<addmax){  //如果小于1050自增到最大
																		value+=addamount;
													 }
										}else{//如果是高速
												  if(value<addmax){  //如果小于1050自增到最大
														...
														if(value>1050){value=1050;}     //高速防溢出
										}
							}
							else{  //如果是负数
										if(addamount){//如果是低速
											   if(value>0){    //负数变大是自减到0
																	value-=addamount;
														}
														else{    //如果是0 当次自增1 变为正号进入正加
																	value = 1;
																	Set_sign=1;
														}
										}else{//如果是高速
											   if(value>0){    //负数变大是自减到0
																	value-=addamount;
																	if(value>addmax){value=0;}//高速防溢出
														}
														else{    //如果是0 当次自增10 变为正号进入正加
																	value = 10;
																	Set_sign=1;
														}
										}
						  ...
						  if(Set_sign) { //如果是正数
										 if(addamount){//如果是低速
											   if(value>0){    //正数自减到0
														...
														else{    //如果是0 当次自增1 变为负号进入负数减法
																	value = 1;
																	Set_sign=0;
														}
										}else{//如果是高速
											   if(value>0){    //正数变小是自减到0
																	value-=addamount;
																	if(value>addmax){value=0;}//高速防溢出
														}
														else{    //如果是0 当次自增10 变为负号进入负减
																	value = 10;
																	Set_sign=0;
														}
										}
								   ...
								   if(addamount==adddeta1)//如果用低速
										{
														if(value<addmax){  //负数自减是绝对值增加到最大
																		value+=addamount;
													 }
										}else{//如果是高速
												  if(value<addmax){  //负数减少等于绝对值自增最大
																		value+=addamount;
													 }
														if(value>1050){value=1050;}     //高速防溢出
										}	
		 ...
		 switch(runsetdisplay)   //按下mode显示设置哪个口
								{
									case dispset_P1:{
									    Delaycheck(1000)
													{
																DIV_Disp_SecondDispSetnum(Set_sign,P1_Value);
																runsetdisplay&=~(dispset_P1);
													}
									}break;
									case dispset_P2:{
									    Delaycheck(1000)
													{
																DIV_Disp_SecondDispSetnum(Set_sign,P2_Value);
																runsetdisplay &= ~(dispset_P2);
													}
									}break;
									case dispset_Hi1:{
									    Delaycheck(1000)
													{
																DIV_Disp_SecondDispSetnum(Set_sign,Hi1_Value);
																runsetdisplay &= ~(dispset_Hi1);
													}
									}break;
									case dispset_Lo1:{
									    Delaycheck(1000)
													{
																DIV_Disp_SecondDispSetnum(Set_sign,Lo1_Value);
																runsetdisplay &= ~(dispset_Lo1);
													}
									}break;
									case dispset_Hi2:{
									    Delaycheck(1000)
													{
																DIV_Disp_SecondDispSetnum(Set_sign,Hi2_Value);
																runsetdisplay &= ~(dispset_Hi2);
													}
									}break;
									case dispset_Lo2:{
									    Delaycheck(1000)
													{
																DIV_Disp_SecondDispSetnum(Set_sign,Lo2_Value);
																runsetdisplay &= ~(dispset_Lo2);
													}
									}break;
								}
...
{  	
        ...
        TMR_ClearIntFlag(TMR2, TMR_INT_FLAG_UPDATE);
				    ...
				    PWM_ONEGED.Pwm_Tick++;  
								...
								//可查询延时时间戳
								encheck_delay.Encheck_Delay_Tick++;
				
								//长按连续设置
								if(Set_2sLongPressEv==2)      //key2长按
								{	
													addordownflag=add;//增加
													if(presstime<changetime){presstime++;}//记录按下时间  时间释放解除
													else if(presstime==changetime&&!(nowvalue%10)){ 									//达到5s不再自增 长按改变速度,且判断是否有余数
															 addamount=adddeta10;//改变增加间隔 时间1s自增，暂时没修该自增间隔
													}
													addcount++;
									    if(addcount==addtime)//达到间隔自增
													{
															addcount=0;
															addflag=1;
													}
								...
								else if(Set_2sLongPressEv==3)      //key3长按
								{
													addordownflag=down;//减号
													if(presstime<changetime){presstime++;}//记录按下时间  时间释放解除
													else if(presstime==changetime&&!(nowvalue%10)){ 									//达到5s不再自增 长按改变速度,且判断是否有余数
															 addamount=adddeta10;//改变增加间隔 时间1s自增，暂时没修该自增间隔
													}
