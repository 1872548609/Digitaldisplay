...
//最小二=======================================
float x[] = {3393.0,2686.0,2228.0,1870.0,1559.0,1066.0,556.0 };
float y[] = {90.35 ,50.37 ,20.33 ,-0.27  ,-20.07,-50.26,-82.47};
...
typedef struct {
				float slope;        // 斜率
    float intercept;    // 截距
    float min_pressure; // 该区间气压值的最小值
    float max_pressure; // 该区间气压值的最大值
} LinearParams;

// 计算六段线性区间的斜率和截距
LinearParams* Calculatesix_linearintervals(float y[], float x[], int data_size) {
    // 假设将数据分为六段，计算每段的起始和结束索引
    int num_intervals = 6;//七个数据六段
	
	   if (data_size < num_intervals) {
        return NULL; // 数据太少，无法分割
    }
				
    int interval_size = data_size / num_intervals;
				//分配一个六段结构体数组
    LinearParams* params = (LinearParams*)malloc(num_intervals * sizeof(LinearParams));
 
				if(!params){    //分配失败返回
						return NULL;
				}
	
    for (int i = 0; i < num_intervals; i++) {        //找到每段的起始结束索引
        int start_index = i * interval_size;
        int end_index = (i + 1) * interval_size;
        if (i == num_intervals - 1) {     //如果不够分割就把剩下的都规划到最后一段
            end_index = data_size-1; // 确保最后一段包含所有剩余数据
        }
 
        float sum_x = 0, sum_y = 0, sum_xy = 0, sum_x2 = 0; //最小二计算斜率
        int count = 0;
        ...
        float x_mean = sum_x / count;
        float y_mean = sum_y / count;
        ...
        float denominator = 0;
        for (int j = start_index; j <= end_index; j++) {
            denominator += (x[j] - x_mean) * (x[j] - x_mean);
        }
								
							 // 计算斜率
        params[i].slope = numerator / denominator;
 
        // 计算截距
        params[i].intercept = y_mean - params[i].slope * x_mean;

								//区间最小气压
								 params[i].min_pressure =y[end_index];
								
								//区间最大气压
								 params[i].max_pressure =y[start_index];
 ...
 float calculate_pressure_from_ad(LinearParams* params, int num_intervals, float ad_value) {
    for (int i = 0; i < num_intervals; i++) {
        // 计算当前区间下的气压值
        float calculated_pressure = params[i].slope * ad_value + params[i].intercept;
 
        // 检查计算出的气压值是否在该区间的气压范围内
        if (calculated_pressure >= params[i].min_pressure && calculated_pressure <= params[i].max_pressure) {
            return calculated_pressure; // 返回匹配的气压值
        }
    }
 
    // 如果所有区间都不匹配，返回错误
    printf("Error: AD value %.2f does not belong to any valid interval!\n", ad_value);
    return -1.0f; // 返回错误值
}
//====================================================================================
//=                                                                             					=
...
//			read_date(&datahander);//把预设值读入
//			manager.write_offset=(datahander.rank)*sizeof(datahander) ;  //要用的是序号的下一位，所以要加一，只用开机自增就可以了，不然启动第一个序号是0，然后偏移也是空，这里要加载偏移
//							
...
//						}
						...
						int data_size = sizeof(y) / sizeof(y[0]);
						
						LinearParams* params = Calculatesix_linearintervals(y, x, data_size);

						for (int i = 0; i < 6; i++) {
						//				printf("Interval %d: slope = %f, intercept = %f , max_pressure = %f ,min_pressure = %f\n", i + 1, params[i].slope, params[i].intercept ,params[i].max_pressure ,params[i].min_pressure);
						}
						
						float press = calculate_pressure_from_ad(params,6,adcData);
					
				  //printf ("nowpress=%f",press);
						
						UnitChangeDisp(MainScreen,press);
...
//								}
								 ...
								 adc_temp =ADC_ReadConversionValue();
									...
									if(adc_temp<adcData-2||adc_temp>adcData+2)
									{
									  ...
									  adc_sum1 += adc_temp1;
									  adc_count++;
           if(adc_count==ADAverage_number)
											{
												adcData= adc_sum1/ADAverage_number;   
												adc_count=0;
												adc_sum1=0; 
											}
