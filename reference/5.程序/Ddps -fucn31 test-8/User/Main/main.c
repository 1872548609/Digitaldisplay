...
volatile uint8_t ECO_delay_start=0;
...
void ECOCtr(void)
{
    ...
    if(ECO_delay_flag)
    {
        ...
        if(PWM_THREERED.compare) threered_pwmval = ecolevel;
        if(PWM_ONERED.compare) onered_pwmval = ecolevel;
        if(PWM_DOUBGED.compare) doubleged_pwmval = ecolevel;
        if(PWM_ONEGED.compare) oneged_pwmval = ecolevel;
...
{
	if(manu1_devicestate!=status_init) //初始化不显示
	{
	 //out12状态显示控制
		if(out1_state)
		{
