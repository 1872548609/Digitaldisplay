...
TestFlash Test_Flash1={0};
TestFlash Test_Flash2={0};

uint8_t DIV_10MS;

//FMC_ProgramHalfWord(uint32_t waddr,uint16_t pbuf);        //把一个16位数据写入一个地址里
//FMC_ProgramWord(uint32_t waddr,uint32_t pbuf); //写一个字
//状态枚举=================================
typedef enum{
			status_init,
			status_run,
...
//				stmflash_read(FLASH_SAVE_ADDR,dataemp,SIZE);
    ...
    FMC_Unlock(); /* FLASH 解锁 */
				Test_Flash1.a=1.22;
				FMC_ErasePage(FLASH_SAVE_ADDR); 
				FMC_ProgramWord(FLASH_SAVE_ADDR,Test_Flash1.b);
	   Test_Flash2.b= *(volatile uint32_t *)FLASH_SAVE_ADDR;
				FMC_Lock(); /* 上锁 */
	...
	switch(mode)
	{
       ...
       HAL_Disp_WrSeriesData(dataid,0x00,Disp_OneFrame,Disp_Max_Length);
		...
		case status_run:{
				...
				HAL_Disp_WrSeriesData(dataid,0x00,OneFrame,Disp_Max_Length);
