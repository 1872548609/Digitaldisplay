            ...
            Delayck(1000)//短按延时关闭lock提示
								...
								else //按键未锁定 正常执行run内容
								{
									...
									CLear_NowSetSHow();//清除显示runset  
										...
										ReadNowPressValue();//读取气压		
									
								 //不同单位不同运行模式，选择应差，选择设定值，选择比较单位  
										unitchange_pressure_value= UnitChange(unitchange_pressure_value);    //单位转换
...
void Flash_Read_SetValue(void)
{
	   ...
	   dispeed							=(*(volatile uint32_t*)FLASH_SAVE_ADDR_PROSET)&0x70;
	   diflevel						=(*(volatile uint32_t*)FLASH_SAVE_ADDR_PROSET)&0x380;
	   colorreaction	=(*(volatile uint32_t*)FLASH_SAVE_ADDR_PROSET)&0xc00;
	   ecomode							=(*(volatile uint32_t*)FLASH_SAVE_ADDR_PROSET)&0x3000;
    facrecover				=(*(volatile uint32_t*)FLASH_SAVE_ADDR_PROSET)&0xc000;                                                      
}
//====================================================================================
//=                                                                             					=
//=   长按设定                                                                	=
